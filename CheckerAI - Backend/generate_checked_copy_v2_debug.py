...
def _plan_annotations_from_ocr(
    ...
    if ocr_page_text.strip():
        ...
        if content_idxs:
            ...
            for line_idx in selected:
                ...
                span = slice_bot - slice_top
                y_frac = min(max(y_frac, slice_top + 0.2 * span), slice_bot - 0.2 * span)
    ...
    if not y_candidates and text_blocks:
        ...
        if slice_blocks:
            ...
            # Sample evenly from the detected ink blocks in our slice
            if max_ann == 1:
                blk_center = slice_blocks[len(slice_blocks) // 2][0]
                y_candidates.append(min(max(blk_center, slice_top), slice_bot - 0.02))
            else:
                step = max(1, len(slice_blocks) / max_ann)
                for i in range(min(max_ann, len(slice_blocks))):
                    blk_center = slice_blocks[int(i * step)][0]
                    y_candidates.append(min(max(blk_center, slice_top), slice_bot - 0.02))

    # ── Step 3: absolute fallback — evenly spaced within the slice ────────────
    if not y_candidates:
        span = slice_bot - slice_top
        if span <= 0.40:
            max_ann = 1
        for i in range(max_ann):
            # Place in the central area of the slice
            progress = (i + 0.5) / max_ann
            y_candidates.append(slice_top + (0.2 + 0.6 * progress) * span)
    ...
    for page_idx in range(num_pages):
        ...
        for item in items:
            ...
            if is_first:
                ...
                # Add any physically-detected headings on this page that are not in items
                # (these are unanswered questions whose heading occupies page space).
                _items_qnums = {it["q_num"] for it in first_items_on_page}
                _phantom_slots = [
                    {"q_num": qn, "is_first": True, "_phantom": True}
                    for qn in pre_heading_fracs
                    if qn not in _items_qnums
                ]
                first_items_on_page = first_items_on_page + _phantom_slots
                if len(first_items_on_page) > 1:
                    # Filter out ghost items (assigned here but no physical heading found)
                    actual_firsts = [it for it in first_items_on_page if it["q_num"] in pre_heading_fracs]
                    if actual_firsts:
                        first_items_on_page = actual_firsts

                if len(first_items_on_page) > 1:
                    first_items_on_page = sorted(
                        first_items_on_page,
                        key=lambda it: pre_heading_fracs.get(it["q_num"], 0.5)
                    )
                    _sorted_q_names = [it['q_num'] for it in first_items_on_page]
                ...
                if ocr_page_text and n_firsts == 1:
                    ...
                    # ── Multiple questions on same page: EQUAL-DIVISION slicing ────────
                    first_q_nums_ordered = [it["q_num"] for it in first_items_on_page]
                    my_idx = first_q_nums_ordered.index(q_num)

                    ink_span = max(0.05, ink_bot - ink_top)
                    slice_h  = ink_span / n_firsts

                    q_top_frac  = ink_top + my_idx * slice_h
                    q_bot_frac  = ink_top + (my_idx + 1) * slice_h
                    # Never exceed actual ink region
                    q_top_frac  = min(q_top_frac, ink_bot)
                    q_bot_frac  = min(q_bot_frac, ink_bot)
                    
                    if n_firsts == 2:
                        multi_q_target_y_frac = 0.16 if my_idx == 0 else 0.82
                    elif n_firsts == 3:
                        multi_q_target_y_frac = [0.16, 0.55, 0.82][my_idx]
                    else:
                        multi_q_target_y_frac = 0.5
                else:
                ...
                if multi_q_target_y_frac is not None:
                    ...
                    print('DEBUG clear_xy finding... q_top_frac:', q_top_frac if 'q_top_frac' in locals() else 'None', 'q_num:', q_num)
                        gray, img_w, img_h, pdf_w, pdf_h,
                    ...
                    print("DEBUG stamp_row_top:", stamp_row_top, "stamp_row_bot:", stamp_row_bot, "q_num:", q_num)
                    ...
                    if margin_spot:
                        ...
                        print('DEBUG margin_spot y_placed:', marks_y_placed, 'q_num:', q_num)
                ...
                print('DEBUG BEFORE CLAMP marks_y_placed:', marks_y_placed, 'q_num:', q_num)
            ...
            if _pending_stamp is not None:
                ...
                if fb_y is not None and n_firsts == 1:
            ...
            for ann in annotations:
                ...
                if ann["action"] == "tick":
                    ...
                    _draw_cross(c, draw_x, draw_y, size=sz)
